...
手柄连接后如出现按键/摇杆无响应，可先运行独立诊断脚本（无需启动 GMS）：

```
python diagnose_gamepad.py [手柄索引]
```

脚本同时用三条链路读取手柄（XInput 原生直读 / SDL 事件缓存 / Windows HID 语义解码），
10 秒内依次按 A/B/X/Y、肩键、推动摇杆与扳机，输出哪一路有数据、哪一路无数据，
可直接定位问题层。
...
### beta0.0.4
- 手柄引擎支持热插拔自动重连：启动后无需先连手柄，连接/断开自动识别，
  蓝牙手柄休眠后唤醒即可继续演奏（修复"连不上/映射不了"）
- Xbox 兼容手柄优先使用 XInput 原生状态（按键、摇杆、扳机、十字键同一帧读取），
  SDL 仅用于发现/热插拔与非 XInput 设备；Windows HID 语义解码保留为显式兜底
- 静止不再被误判为断线，不会在 15 秒无操作后主动拆除手柄句柄
- 按钮/轴布局感知：连接时自动查询 SDL 控制器映射表解析按钮与扳机原始索引，
  内置 Xbox One/Series 家族 HID 布局表（含蓝牙 BLE），修复蓝牙 Xbox 手柄
  L3/R3 错位（Guide 误当 L3、R3 无响应）；`l3_button`/`r3_button` 改为
  -1=自动，可手动指定，旧配置 8/9 自动迁移
- 修复手柄断开时可能残留卡音问题（自动释放全部按住的音符）
- 主控台新增"等待手柄…"状态实时刷新，按钮可视化按实际布局动态标注
